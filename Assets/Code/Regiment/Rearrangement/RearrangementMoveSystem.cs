    ...
    public partial class RearrangementMoveSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            .ForEach((Entity unit, ref Translation position, ref Data_AnimationPlayed anim ,in Data_Regiment regiment, in Data_IndexInRegiment indexInRegiment) =>
            {
                ...
                float2 direction =  math.normalizesafe(destination.xz - position.Value.xz);
                float3 newPosition = math.mad(new float3(direction.x, 0.0f, direction.y), deltaTime*2, position.Value);
                
                position.Value = newPosition;
                
                float dst = math.distance(destination, position.Value);
                if (dst <= 0.06f)
                {
